    ...
    try {
      ...
      if (rawList != null) {
        for (final entry in rawList) {
          ...
          try {
            final contentResponse = await _apiService.getContentById(contentId as int);
            final merged = Map<String, dynamic>.from(contentResponse);
            final lastPosition = entry['lastPositionSeconds'];
            if (lastPosition is int) {
              merged['resumePositionSeconds'] = lastPosition;
            }
            items.add(Content.fromJson(merged));
          } catch (e) {
            if (kDebugMode) {
              print('Failed to load content for continue watching: $e');
            }
          }
        }
      }
